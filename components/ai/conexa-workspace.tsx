...
import { BarChart3, ChevronDown, CircleHelp, Database, Gauge, GitBranch, LayoutDashboard, MessageCircle, PanelLeftClose, PanelLeftOpen, Search, Settings2, Sparkles, Tags, Users, WalletCards, X } from 'lucide-react'
...
type Platform = { name: string; key: string; icon: typeof BarChart3; color: string; connected: boolean; detail: string }
...
export function ConexaWorkspace() {
  ...
  const platforms = useMemo<Platform[]>(() => [
    { name: 'Meta Ads', key: 'meta', icon: BarChart3, color: '#1877F2', connected: Boolean(client?.meta_ads_account_id), detail: client?.meta_ads_account_id ? `Cuenta ${client.meta_ads_account_id}` : 'Sin cuenta conectada' },
    { name: 'Google Ads', key: 'google', icon: Search, color: '#4285F4', connected: Boolean(client?.google_ads_customer_id), detail: client?.google_ads_customer_id ? `Cuenta ${client.google_ads_customer_id}` : 'Sin cuenta conectada' },
    { name: 'Google Analytics', key: 'analytics', icon: Gauge, color: '#F9AB00', connected: Boolean(client?.analytics_property_id), detail: client?.analytics_property_id ? `Propiedad ${client.analytics_property_id}` : 'Sin propiedad conectada' },
    { name: 'Tag Manager', key: 'tag_manager', icon: Tags, color: '#246FDB', connected: Boolean(client?.tag_manager_container_id), detail: client?.tag_manager_container_id ? `Contenedor ${client.tag_manager_container_id}` : 'Sin contenedor conectado' },
    { name: 'CRM Aurelia', key: 'crm', icon: Users, color: '#11A683', connected: Boolean(client?.crm_type), detail: client?.crm_type ? `Conectado · ${client.crm_type}` : 'Sin CRM conectado' },
  ], [client])
