...
function PlatformView({ platform }: { platform: Platform }) {
  ...
  return <div className="mx-auto max-w-[1180px] px-6 py-5"><div className="mb-3 flex items-start justify-between"><div className="flex items-center gap-3"><span className="flex size-9 items-center justify-center overflow-hidden rounded-lg bg-white">{platform.iconUrl ? <img src={platform.iconUrl} alt="" className="size-full object-contain" /> : <Globe2 className="size-7 text-[#11A683]" />}</span><div><h1 className="text-[20px] font-bold">{platform.name}</h1><p className="text-[11px] text-[#777]">{platform.detail} <span className="ml-2 text-[#1e9e6b]">● Conectado</span></p></div></div><Button variant="outline" className="h-8 rounded-full px-4 text-[11px]">Administrar conexión</Button></div><div className="mb-4 flex gap-7 border-b border-[#dededb] text-[10px] font-semibold text-[#777]">{config.tabs.map((tab, index) => <button type="button" key={tab} className={cn('border-b-2 px-1 pb-3', index === 0 ? 'border-[#5b5fe8] text-[#5b5fe8]' : 'border-transparent')}>{tab}</button>)}</div><div className="mb-4 grid grid-cols-6 gap-3">{config.metrics.map(([label, value]) => <div key={label} className="rounded-xl border border-[#dcdcd8] bg-white p-3 shadow-[0_2px_5px_rgba(0,0,0,.06)]"><p className="text-[10px] text-[#888]">{label}</p><p className="mt-1 text-[16px] font-bold">{value}</p></div>)}</div><div className="mb-4 rounded-xl border border-[#dcdcd8] bg-white p-4"><p className="mb-3 text-[10px] font-bold uppercase text-[#999]">{config.chart}</p><div className="flex h-28 items-end gap-2">{[42, 55, 35, 70, 62, 78, 52, 65, 48, 72, 60, 84, 67, 76].map((height, index) => <div key={index} className="flex-1 rounded-t-sm bg-[#8587e8]" style={{ height: `${height}%` }} />)}</div></div><div className="rounded-xl border border-[#dcdcd8] bg-white"><div className="grid grid-cols-5 border-b border-[#e9e9e6] px-4 py-3 text-[10px] font-bold text-[#888]"><span>Entidad</span><span>Estado</span><span>Objetivo</span><span>Resultados</span><span className="text-right">Acción</span></div>{config.rows.map((row, index) => <div key={row} className="grid grid-cols-5 items-center border-b border-[#f0f0ee] px-4 py-3 text-[11px] last:border-0"><span className="font-semibold">{row}</span><span className="text-[#1e9e6b]">{platform.connected ? 'Activo' : 'Pendiente'}</span><span>{index === 0 ? 'Leads' : 'Conversiones'}</span><span>{[128, 91, 64][index] ?? 42}</span><button type="button" className="text-right font-bold text-[#5b5fe8]">✦ Analizar</button></div>)}</div></div>
}
