...
export function ConexaWorkspace() {
  ...
  const platforms = useMemo<Platform[]>(() => [
    { name: 'Meta Ads', key: 'meta', icon: BarChart3, color: '#1877F2', iconUrl: 'https://hebbkx1anhila5yf.public.blob.vercel-storage.com/Facebook_Logo_%282019%29-hJ3k3NCNwb5LRvxXF8cWFtCFvOnZU2.png', connected: Boolean(client?.meta_ads_account_id), detail: client?.meta_ads_account_id ? `Cuenta ${client.meta_ads_account_id}` : 'Sin cuenta conectada' },
    ...
    { name: 'CRM Aurelia', key: 'crm', icon: Users, color: '#11A683', iconUrl: 'https://hebbkx1anhila5yf.public.blob.vercel-storage.com/6033716-4LKa2ajYtiv022xYzJ7KZVhigtgjZW.png', connected: Boolean(client?.crm_type), detail: client?.crm_type ? `Conectado · ${client.crm_type}` : 'Sin CRM conectado' },
  ], [client])
...
function Home({ client, platforms, connected, onAsk }: { client: Client | null; platforms: Platform[]; connected: number; onAsk: () => void }) {
  ...
  return <div className="mx-auto max-w-[1180px] px-8 py-8">
    ...
    <div className="mb-6 grid grid-cols-[220px_1fr] gap-4"><div className="rounded-2xl bg-[#141414] p-5 text-white"><p className="text-[11px] text-white/60">SCORE GENERAL</p><div className="mt-3 text-[48px] font-bold leading-none">{connected === platforms.length ? 100 : connected * 20}</div><p className="mt-2 text-xs text-white/60">Conexiones, tracking y alertas</p></div><div className="rounded-2xl border border-[#e6e6e3] bg-white p-[18px_22px]">{platforms.map((item) => <div key={item.key} className="flex items-center justify-between py-1 text-[13.5px]"><span className="flex items-center gap-2 font-medium"><span className="flex size-6 items-center justify-center overflow-hidden rounded-md bg-white"><img src={item.iconUrl} alt="" className="size-full object-contain" /></span>{item.name}</span><span className={cn('flex items-center gap-1.5 font-medium', item.connected ? 'text-[#1e9e6b]' : 'text-[#9a9a9a]')}><span className={cn('size-1.5 rounded-full', item.connected ? 'bg-[#1e9e6b]' : 'bg-[#d6d6d6]')} />{item.connected ? 'Conectado' : 'Pendiente'}</span></div>)}</div></div>
