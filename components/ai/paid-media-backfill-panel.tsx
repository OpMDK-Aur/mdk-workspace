  ...
  return (
    <Card className="overflow-hidden">
      ...
      {open && <CardContent className="flex flex-col gap-4 border-t pt-4">
        ...
        {!!result?.errors?.length && <div className="rounded-md border border-destructive/30 bg-destructive/5 p-3 text-sm text-destructive"><p className="font-medium">Errores</p><ul className="mt-1 list-disc pl-5">{result.errors.map((error, index) => <li key={`${error}-${index}`}>{error}</li>)}</ul>{result.persistence_errors?.map((error, index) => <div key={`${error.date}-${index}`} className="mt-3 rounded border border-destructive/20 p-2 text-xs"><p><strong>{error.error_code}</strong> · {error.platform} · {error.account_id} · {error.date}</p><p>Mensaje: {error.error_message || '—'}</p><p>Detalles: {error.error_details || '—'}</p><p>Hint: {error.error_hint || '—'}</p>{error.validation_errors?.length ? <p>Campos inválidos: {error.validation_errors.join(', ')}</p> : null}</div>)}</div>}
