...
export function ServiceMapMilestoneEditor() {
  ...
  const chooseCurrent = (instance: typeof currentMilestones[number]) => {
    const item = items.find((candidate) => candidate.id === instance.hito?.id)
    if (item) choose(item)
    else setNotice('Este hito ya no existe en el catálogo.')
  }
