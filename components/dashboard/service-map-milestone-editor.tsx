...
import { Save, Trash2, Loader2, Check, X } from "lucide-react";
...
const frequencies: FrecuenciaHito[] = [
  "Mensual",
  "Bimestral",
  "Semanal",
  "Semanal (Lun)",
...
export function ServiceMapMilestoneEditor() {
  ...
  const choose = (item: HitoCatalogo) => {
    setSelected(item);
    setDraft({ ...item });
    setNotice("");
  };
  ...
  const save = async () => {
    ...
    if (!clientId)
      return setNotice(
        "Seleccioná un cliente para guardar y actualizar sus hitos.",
      );
    setSaving(true);
    const payload = {
      ...draft,
    ...
    const query = selected
      ? supabase.from("hitos_catalogo").update(payload).eq("id", selected.id)
      : supabase.from("hitos_catalogo").insert(payload);
    const { error } = await query;
    if (error) {
      setSaving(false);
      return setNotice(`No se pudo guardar: ${error.message}`);
    }
    const now = new Date();
    ...
    const taskIds = (instances ?? [])
      .map((instance) => instance.tarea_id)
      .filter((id): id is string => Boolean(id));
    if (taskIds.length)
      await supabase.from("tareas").delete().in("id", taskIds);
    ...
    const result = await generateMonthInstances(
      ...
      month,
      year,
      (client?.plan ?? draft.tipo_servicio) as any,
    );
    setSaving(false);
    if (!result.success)
      return setNotice(
        `Hito guardado, pero no se pudo actualizar el mes corriente: ${result.error ?? "error desconocido"}`,
      );
    setNotice(
      "Cambios guardados y aplicados desde el mes corriente. Los meses anteriores no fueron modificados.",
    );
    setSelected(null);
    setDraft(blank);
    await load();
  ...
  const remove = async () => {
    ...
    setNotice(
      "Hito borrado del catálogo. Las tareas históricas no se modifican.",
    );
    setSelected(null);
    setDraft(blank);
    await load();
      ...
      <CardContent className="flex flex-col gap-6">
        ...
        <div className="hidden">
          <div className="flex flex-col gap-3">
            <div>
              <h3 className="font-semibold">Catálogo de hitos</h3>
              <p className="text-sm text-muted-foreground">
                Elegí un hito para editarlo o creá uno nuevo.
              </p>
            </div>
            <div className="flex max-h-[32rem] flex-col gap-2 overflow-auto">
              {items.map((item) => (
                <button
                  ...
                  key={item.id}
                  onClick={() => choose(item)}
                  className="flex items-start justify-between rounded-lg border p-3 text-left hover:bg-muted"
                >
                  <span>
                  ...
                  <Badge variant="outline">#{item.orden}</Badge>
                </button>
              ))}
            </div>
          </div>
          <div className="flex flex-col gap-4 rounded-lg border bg-card p-4 shadow-sm">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <label className="flex flex-col gap-2 text-sm">
                ...
                <Select
                  ...
                  onValueChange={(value) =>
                    setDraft({ ...draft, tipo_servicio: value as TipoServicio })
                  }
                >
                  <SelectTrigger>
                    <SelectValue />
              ...
              <label className="flex flex-col gap-2 text-sm">
                ...
                <Select
                  ...
                  onValueChange={(value) =>
                    setDraft({ ...draft, frecuencia: value as FrecuenciaHito })
                  }
                >
                  <SelectTrigger>
                    <SelectValue />
            ...
            <label className="flex items-center gap-2 text-sm">
              <Checkbox
                ...
                onCheckedChange={(checked) =>
                  setDraft({ ...draft, requiere_link_drive: checked === true })
                }
              />
