...
export async function GET(request: Request) {
  ...
  const { data, error } = await supabase.from('ai_client_profile').select('industry, commercial_objective, product_type, primary_conversion_type, industry_source, commercial_objective_source, product_type_source, primary_conversion_source').eq('client_id', clientId).maybeSingle()
