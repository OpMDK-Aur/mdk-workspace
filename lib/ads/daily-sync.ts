...
function rowsForAccount(account: Account, metrics: any, date: string) {
  if (account.plataforma === 'google') return metrics.campaigns.map((campaign: any) => { const results = finiteMetric(campaign.leads ?? campaign.conversions ?? campaign.results); return { client_id: account.cliente_id, platform: 'google', account_id: account.id_cuenta.replace(/-/g, ''), account_name: normalizeOptional(account.nombre_cuenta), metric_date: date, campaign_id: campaign.id, campaign_name: normalizeOptional(campaign.name), campaign_type: normalizeOptional(campaign.advertising_channel_type), campaign_objective: null, result_type: 'lead', currency: normalizeOptional(account.moneda ?? 'ARS'), spend: finiteMetric(campaign.spend), impressions: finiteMetric(campaign.impressions), clicks: finiteMetric(campaign.clicks), results, leads: results, conversions: results, ctr: normalizeOptional(campaign.ctr), cpc: normalizeOptional(campaign.cpc), cost_per_result: normalizeOptional(campaign.cpl), source: 'google_ads' } })
