...
function buildPrompt(context: ExecutionContext, currentSnapshots: PaidMediaSnapshot[], comparisonSnapshots: PaidMediaSnapshot[], changeHistory: PaidMediaChangeEvent[]) {
  ...
  return JSON.stringify({
    ...
    optimizationDefinitions: context.scoreConfig?.descriptions ?? null,
    reglas: [
      ...
      'Incluí optimization_level como baja, intermedia o alta y optimization_score de 0 a 100 cuando haya evidencia suficiente. Explicá y priorizá recomendaciones accionables.',
  ...
  try {
    result = await generateText({
      ...
      system: [
        ...
        `Clasificación configurable de optimización: BAJA = ${context.scoreConfig?.descriptions.low ?? 'evidencia insuficiente o rendimiento crítico'}; INTERMEDIA = ${context.scoreConfig?.descriptions.intermediate ?? 'señales mixtas'}; ALTA = ${context.scoreConfig?.descriptions.high ?? 'buen rendimiento y eficiencia'}. Clasificá con evidencia y entregá recomendaciones accionables basadas en los hallazgos.`,
