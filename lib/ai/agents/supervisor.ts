...
export async function streamSupervisorResponse(
  ...
  return streamText({
    ...
    system: [
      ...
      'ORQUESTACIÓN: preguntas contextuales o factuales responden con la tool de lectura correspondiente y no requieren Performance Analyst. Preguntas de diagnóstico o performance requieren métricas y luego run_performance_analyst. Preguntas de variación requieren current + comparison válidos antes del análisis. Preguntas de historial usan get_account_change_history y no requieren especialista salvo que también pidan impacto o causalidad. Preguntas causales requieren métricas, comparación, historial y run_performance_analyst; nunca presentes una correlación temporal como causa confirmada. No inventes findings ni recomendaciones.',
