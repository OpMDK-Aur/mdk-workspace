...
export async function streamSupervisorResponse(
  ...
  const enabledToolKeys = [...new Set([...config.enabledTools, benchmarkToolRequired, crmToolRequired, analyticsSalesToolRequired])]
  ...
  return streamText({
    ...
    system: [
      ...
      'GOOGLE ANALYTICS 4: si el usuario pregunta por ventas, compras, transacciones, ingresos o eventos purchase de GA4, ejecutá obligatoriamente get_google_analytics_sales antes de responder. No digas que GA4 no está disponible sin haber ejecutado esa herramienta; la propiedad se resuelve desde la configuración persistida del cliente.',
